...
def softmax_loss_naive(W, X, y, reg):
    ...
    train_num = X.shape[0]  # num samples
    ...
    loss = loss / train_num + reg * np.sum(W ** 2)  # average loss and regularize
    dW = dW / train_num + 2 * reg * W  # finish calculating gradient
...
def softmax_loss_vectorized(W, X, y, reg):
    ...
    train_num = X.shape[0]  # number of samples
    ...
    loss = -np.log(P[range(train_num), y]).sum()  # sum cross entropies as loss
    loss = loss / train_num + reg * np.sum(W ** 2)  # average loss and regularize

    P[range(train_num), y] -= 1  # update P for gradient
    dW = X.T @ P / train_num + 2 * reg * W  # calculate gradient
